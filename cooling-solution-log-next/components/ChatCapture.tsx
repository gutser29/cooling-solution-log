...
export default function ChatCapture({ onNavigate }: ChatCaptureProps) {
  ...
  useEffect(() => {
    const loadContext = async () => {
      try {
        const recentEvents = await db.events.orderBy('timestamp').reverse().limit(30).toArray()
        if (recentEvents.length > 0) {
          const contextSummary = recentEvents.map(e => {
            const date = new Date(e.timestamp).toLocaleDateString()
            const pm = e.payment_method || 'N/A'
            return `[${date}] ${e.category || e.subtype} - $${e.amount} (${pm}) - ${e.vendor || e.note || ''}`
          }).join('\n')
          console.log('Contexto cargado:', recentEvents.length, 'eventos')
          setMessages(prev => [...prev, { role: 'user', content: `CONTEXTO_INICIAL: Estos son mis últimos registros:\n${contextSummary}\n\nAhora estoy listo para nuevos registros.` }, { role: 'assistant', content: '✅ Contexto cargado. Ya recuerdo tus últimos movimientos. ¿Qué registramos ahora?' }])
        }
      } catch (error) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (data?.type === 'GENERATE_PDF') {
        ...
        console.log('Rango de fechas:', new Date(startDate), 'a', new Date(endDate))
        generateCategoryReport(allEvents, pdfData.category || 'general', startDate, endDate)
        setMessages(prev => [...prev, { role: 'assistant', content: `✅ Reporte generado: ${pdfData.category || 'general'} (${pdfData.period || 'custom'})` }])
        return
      }
      ...
      if (saveMatch) {
        try {
          ...
          const savedEvent = { timestamp: eventData.timestamp || Date.now(), type: (eventData.type || 'expense') as 'expense' | 'income', status: 'completed' as 'pending' | 'completed', subtype: eventData.subtype || '', category: eventData.category || '', amount: Number(eventData.amount || 0), payment_method: eventData.payment_method || '', vendor: eventData.vendor || '', vehicle_id: eventData.vehicle_id || '', client: eventData.client || '', note: eventData.note || eventData.description || '', raw_text: assistantText }
          console.log('Evento normalizado:', savedEvent)
          await db.events.add(savedEvent)
          setMessages(prev => [...prev, { role: 'assistant', content: `✅ Guardado: ${eventData.category || ''} $${eventData.amount || 0}\n\n¿Algo más?` }])
          return
        } catch (e) {
          console.error('Error parseando SAVE_EVENT:', e)
          setMessages(prev => [...prev, { role: 'assistant', content: '❌ Error al guardar (JSON inválido). Pruébalo de nuevo.' }])
          return
        }
      }
    ...
    <div className="flex flex-col h-screen bg-gray-100 text-gray-900 relative dark:bg-[#0b1220] dark:text-gray-100">
      ...
      {showMenu && (
        <>
          ...
          <div className="absolute top-16 right-4 bg-white dark:bg-[#111a2e] rounded-lg shadow-2xl z-50 w-56 overflow-hidden border border-black/10 dark:border-white/10">
            ...
            <button onClick={async () => { setShowMenu(false); const now = new Date(); const category = 'Gasolina'; const startDate = new Date(now.getFullYear(), now.getMonth(), 1).getTime(); const endDate = now.getTime(); const allEvents = await db.events.toArray(); generateCategoryReport(allEvents, category, startDate, endDate); setMessages(prev => [...prev, { role: 'assistant', content: `✅ Reporte generado: ${category} (month)` }]) }} className="block w-full text-left px-4 py-3 hover:bg-gray-100 dark:hover:bg-white/10">Generar Reporte</button>
          </div>
        </>
      )}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-4 shadow-lg dark:bg-[#0f172a] dark:border-white/10">
        <div className="max-w-2xl mx-auto flex gap-2">
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Escribe aquí..." className="flex-1 border rounded-full px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-[#0b1220] dark:border-white/10" disabled={loading} />
          <button onClick={handleSend} disabled={loading || !input.trim()} className="bg-blue-500 text-white rounded-full px-6 py-3 font-bold disabled:bg-gray-300">{loading ? '⏳' : '📤'}</button>
